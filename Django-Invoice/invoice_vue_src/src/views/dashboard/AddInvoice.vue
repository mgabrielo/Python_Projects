<template>
 <div class="page-add-invoice">
        <div class="columns is-multiline">
            ...
            <div class="column is-12">
                <h1 class="is-size-5 mb-4">Add Invoice</h1>
                
                <div class="select">
                    <select name="client" v-model="invoice.client">
                        <option value="">- Select Client -</option>
                        <option v-for="client in clients"
                        v-bind:key="client.id" v-bind:value="client"
                        >{{ client.name }}</option>
                    </select>
                </div>
        ...
        <div class="column is-12">
            <h2 class="button is-light">Total</h2>
            <p><strong>Net Amount:</strong>{{ invoice.net_amount }}</p>
            <p><strong>VAT Amount:</strong>{{ parseInt(invoice.vat_amount) }}</p>
            <p><strong>Gross Amount:</strong>{{ invoice.gross_amount }}</p>
        </div>

        <div class="column is-12">
            <button class="button is-success" @click="submitForm">Save Invoice</button>
        </div>
    </div>
    ...
    methods:{
        ...
        updateTotals(changedItem){
            ...
            for(let i=0; i< this.invoice.items.length; i++){
                ...
                vat_amount += this.invoice.items[i].net_amount * (parseInt(vat_rate/100))
                net_amount +=this.invoice.items[i].net_amount  
            }

            this.invoice.net_amount = net_amount
            this.invoice.vat_amount = vat_amount
            this.invoice.gross_amount = parseInt(vat_amount + net_amount)
        ...
        submitForm(){
            ...
            axios.post('/api/v1/invoices/', this.invoice)
            .then(response=>{
                toast({
